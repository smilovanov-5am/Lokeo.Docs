...
== Получение настроек интернет-эквайринга
* GET mobile/hasInternetAcquiring

=== Ответ
- hasInternetAcquiring - подключена система эквайринга
- hasSBP - есть возможность оплаты через СБП
- hasBankCard - есть возможность оплаты через банковскую карту
...
isSPBPayment - сообщает, что оплата будет идти через СБП
